            ...
            logSize = 0;
                do {
                    ...
                    if (size>0)
                    {
                    	...
                    	{
                    		sDate.day = ApplVar.FlowBuff.CDC_regis.FDay;
        					sDate.month = ApplVar.FlowBuff.CDC_regis.FMonth;
        					sDate.year = ApplVar.FlowBuff.CDC_regis.FYear + 2000;
        					sDate.hour = ApplVar.FlowBuff.CDC_regis.FHour;
        					sDate.min = ApplVar.FlowBuff.CDC_regis.FMinute;
        					sDate.sec = ApplVar.FlowBuff.CDC_regis.FSecond;
                        ...
                        {//有要发送的数据,将数据发送出去
							...
							sprintf(INFO_ALL + strlen(INFO_ALL), "%d-%02d-%02d %02d:%02d:%02d",
								sDate.year, sDate.month, sDate.day, sDate.hour, sDate.min, sDate.sec);
