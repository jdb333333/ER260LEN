...
#define GPRSSENDTXT //jdb2018-08-27发送一张票据
...
#ifdef GPRSSENDTXT //jdb2018-08-27 发送纯文本数据
#define INFO_1 "INFO<NAME>"
#define INFO_2 "</NAME><FS>"
#define INFO_3 "</FS><FDT>"
#define INFO_4 "</FDT><CRC>"
#define INFO_5 "</CRC>"

#define CONTENT_1 "FILE<DATA>"
#define CONTENT_2 "</DATA>"

#define REQUEST "52455155455354"
#define DATASUCCESS "53554343455353"

BYTE CalculateCrcSum(ULONG Size)
{
	char CMD[2048];
    unsigned char bcc;
	int j;
...
BYTE GPRSWaitForRequest(ULONG sWaitFor)
{
    char strOK[512];
    short i,j;
    BYTE sEnd=0;
	...
	if(i >= 7 && strstr(strOK, REQUEST))
			return true;
	
    return false;//false;
}

BYTE GPRSWaitForSuccess(ULONG sWaitFor)
{
    char strOK[512];
    short i,j;
    BYTE sEnd=0;
	...
	if(i >= 7 && strstr(strOK, DATASUCCESS))
			return true;
...
void GPRSSendECR_LOG()
{
	...
	int rpyear, rpmonth, rpday, rphour, rpmin, rpsec;
	BYTE ShopID, CashID;
	BYTE INFO_ALL[200];
	BYTE DATA_BUF[2048];
	BYTE CRCSUM;
        ...
        if (GPRSStartGprs())
        {
            ...
            logSize = 0;
                do {
                	...
                	logReadCopyLast = ApplVar.FlowHeader.ReadP;
                    size = Read_Flow((BYTE *)&ApplVar.FlowBuff);//读取一条流水
                    
                    if (size>0)
                    {
                    	if(ApplVar.FlowBuff.CDC_dpt.FunN == REGISLOG)
                    	{
                    		rpday = ApplVar.FlowBuff.CDC_regis.FDay & 0xff;
        					rpmonth = ApplVar.FlowBuff.CDC_regis.FMonth & 0xff;
        					rpyear = ApplVar.FlowBuff.CDC_regis.FYear & 0xff;
									rpyear += 2000;
        					rphour = ApplVar.FlowBuff.CDC_regis.FHour & 0xff;
        					rpmin = ApplVar.FlowBuff.CDC_regis.FMinute & 0xff;
        					rpsec = ApplVar.FlowBuff.CDC_regis.FSecond & 0xff;

							ShopID = ApplVar.AP.Flag[0x0e];
							CashID = ApplVar.AP.Flag[0x0f];
                    	}
                        if(ApplVar.FlowBuff.CDC_dpt.FunN == RECEIPTLOG)
                    	{
                    		memcpy(PCBuffer+logSize,(BYTE *)ApplVar.FlowBuff.CDC_Receipt.DataBuf,
								ApplVar.FlowBuff.CDC_Receipt.DataLen);
							logSize+=ApplVar.FlowBuff.CDC_Receipt.DataLen;
                    	}
                    }
                        ...
                        {//有要发送的数据,将数据发送出去
                        	//发送包头
                        	memset(INFO_ALL, 0, sizeof(INFO_ALL));
							strcat(INFO_ALL, INFO_1);
							sprintf(INFO_ALL + strlen(INFO_ALL), "%d-%d", ShopID, CashID);
							strcat(INFO_ALL, INFO_2);
							sprintf(INFO_ALL + strlen(INFO_ALL), "%d", logSize);
							strcat(INFO_ALL, INFO_3);
							sprintf(INFO_ALL + strlen(INFO_ALL), "%04d-%02d-%02d %02d:%02d:%02d",
								rpyear, rpmonth, rpday, rphour, rpmin, rpsec);
							strcat(INFO_ALL, INFO_4);
							CRCSUM = CalculateCrcSum(logSize);
							sprintf(INFO_ALL + strlen(INFO_ALL), "0x%02x", CRCSUM & 0xff);
							strcat(INFO_ALL, INFO_5);
							if(!GPRSSendECRData(INFO_ALL,strlen(INFO_ALL)))
							{
								break;
							}

							if (!GPRSWaitForRequest(SECONDS(10)))
							{
                				break;//没有收到 "REQUEST"
							}
							...
							strcat(DATA_BUF, CONTENT_1);
							memcpy(DATA_BUF + strlen(DATA_BUF), PCBuffer, logSize);
							strcat(DATA_BUF, CONTENT_2);
							
                            if (
                                 //!GPRSSendECRData(PCBuffer,logSize)//发送流水数据
                                 !GPRSSendECRData(DATA_BUF,strlen(DATA_BUF))//发送流水数据
                                )
                            {//发送失败时
                            	size = 0;
                            }

							if (!GPRSWaitForSuccess(SECONDS(10)))
							{
                				break;//没有收到 "SUCCESS"
							}
