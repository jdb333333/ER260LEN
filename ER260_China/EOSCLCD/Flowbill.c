...
//jdb2018-08-22 打印票据文本流水
#define ReceiptLOG
extern  BYTE PrBuffer2[];
extern void PrintSaveStr();
...
void DispTAX(void);
void DispRECEIPT(void);//jdb2018-08-20 显示打印票据
...
CONST struct ECRLOGDES LogSize[LOGTYPEMAX]={
    ...
    {sizeof(struct CDC_RECEIPT),         &DispRECEIPT},         //jdb2018-08-20 显示打印票据
#if defined(CASE_INDIA)//ccr2016-06-03>>>>>>>
...
short Read_Flow(BYTE *Area)
{
    ...
    if (i>0 && i<=LOGTYPEMAX)
    {
		...
		//jdb2018-08-22 打印票据文本流水
		else if(i == RECEIPTLOG)
		{
			ReadFrLogRam(Area,ApplVar.FlowHeader.ReadP+1,2);
			sLength = *((WORD *)Area);
			Area++;
			Area++;
			i = 3;
...
//jdb2018-08-20 保存票据内容
short Receipt_Data(BYTE cmd)
{
	#ifdef ReceiptLOG
    struct TimeDate sDate;
	static struct CDC_REGIS CDC_regis;
    ...
    case REGISLOG:
		CDC_regis.FunN = cmd;
        CDC_regis.EcrNumber = ((WORD)LOCATION*256)+REGISTER;
        CDC_regis.OperNo = (BYTE)ApplVar.ClerkNumber;
        CDC_regis.SalesPer = (BYTE)ApplVar.SalPerNumber;

		sBCD = ZERO;
		memcpy(sBCD.Value, ApplVar.ReceiptNumber, 3);
		if (ApplVar.FPb && TESTBIT(PBINFO, BIT4))
            Add(&sBCD, (BCD*)&ONE);
        memcpy(CDC_regis.ReceiptNum, sBCD.Value, sizeof(CDC_regis.ReceiptNum));

        GetTimeDate(&sDate);
        CDC_regis.FDay = sDate.day;
        CDC_regis.FMonth = sDate.month;
        CDC_regis.FYear =  (BYTE)sDate.year;
        CDC_regis.FHour = sDate.hour;
        CDC_regis.FMinute = sDate.min;
        CDC_regis.FSecond = sDate.sec;
        break;
	case RECEIPTLOG:
		Length = sizeof(CDC_regis);
		CheckLogBuff();//使用flash时，会自动擦除存储器，释放空间
		WriteToLogRam(ApplVar.FlowHeader.NextNP,(char *)&CDC_regis,Length);
		ApplVar.FlowHeader.NextNP = ApplVar.FlowHeader.NextNP + Length;
		ADJLOGADDR(ApplVar.FlowHeader.NextNP);
		memset(&CDC_regis, 0, sizeof(CDC_regis));

		CDC_Receipt.FunN = cmd;
		CDC_Receipt.DataLen = strlen(PrBuffer2);
		memcpy(CDC_Receipt.DataBuf, PrBuffer2, CDC_Receipt.DataLen);
		Length = 3 + strlen(CDC_Receipt.DataBuf);
		CheckLogBuff();//使用flash时，会自动擦除存储器，释放空间
		WriteToLogRam(ApplVar.FlowHeader.NextNP,(char *)&CDC_Receipt,Length);
		ApplVar.FlowHeader.NextNP = ApplVar.FlowHeader.NextNP + Length;
		ADJLOGADDR(ApplVar.FlowHeader.NextNP);
		memset(&CDC_Receipt, 0, sizeof(CDC_Receipt));
		break;
    default:
        return 0;
    }
...
//jdb2018-08-20 显示打印票据
void DispRECEIPT()
{
	memcpy(PrBuffer2, ApplVar.FlowBuff.CDC_Receipt.DataBuf, ApplVar.FlowBuff.CDC_Receipt.DataLen);
	PrBuffer2[ApplVar.FlowBuff.CDC_Receipt.DataLen] = '\0';
	PrintSaveStr();
}
...
void LoadReceipLog()
{
    ...
    while (Read_Flow((BYTE *)&ApplVar.FlowBuff)>0)
    {
        ...
        case RECEIPTLOG://jdb2018-08-23 增加打印票据文本功能
        case SUBTOTALLOG:
...
//ccr2016-03-10                    PrintLine('-');
                ...
                if ((sFunc!=ENDLOG || (sLast!=TVOIDLOG && sLast!=PORALOG)) &&   //ccr2016-08-11
                    (LogDefine.LogType==0 || LogDefine.LogType==sFunc || sCorrectLog || sDiscAddLog)
                    #ifdef ReceiptLOG
					&& sFunc != REGISLOG
					#endif
					)//ccr2015-06-15
                ...
                //if (sFunc==TENDLOG)
				if (sFunc==TENDLOG || sFunc == RECEIPTLOG)//jdb2018-08-23 增加打印票据文本功能
                    sActLog = false;
            }
            break;
        }
