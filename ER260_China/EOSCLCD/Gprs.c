...
void GPRSSendECR_LOG()
{
	...
	WORD rpyear;
	BYTE rpmonth, rpday, rphour, rpmin, rpsec;
	BYTE ShopID, CashID;
	BYTE INFO_ALL[200];
	BYTE DATA_BUF[2048];
	BYTE CRCSUM;
            ...
            logSize = 0;
                do {
                    ...
                    if (size>0)
                    {
                    	...
                    	{
                    		rpday = ApplVar.FlowBuff.CDC_regis.FDay;
        					rpmonth = ApplVar.FlowBuff.CDC_regis.FMonth;
        					rpyear = ApplVar.FlowBuff.CDC_regis.FYear;
        					rphour = ApplVar.FlowBuff.CDC_regis.FHour;
        					rpmin = ApplVar.FlowBuff.CDC_regis.FMinute;
        					rpsec = ApplVar.FlowBuff.CDC_regis.FSecond;
                        ...
                        {//有要发送的数据,将数据发送出去
							...
							sprintf(INFO_ALL + strlen(INFO_ALL), "20%02x-%02x-%02x %02x:%02x:%02x",
