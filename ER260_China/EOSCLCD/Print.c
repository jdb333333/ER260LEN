...
BYTE	PrBuffer[2000];//jdb2018-08-21 保存打印文本
BYTE	PrBuffer2[2000];//jdb2018-08-21 保存打印文本
...
void PrintSaveStr()
{
	...
	for(i = 1; i < datalen; i++)
	{
		...
		else
		{
			...
			pribuf[j] = '\0';
			//OutPrint(flag, pribuf);

			if (pribuf && pribuf[0]=='@')
            	OutPrintEx(CMDP_DR, pribuf+1, 0);
        	else
            	OutPrintEx(flag, pribuf, 0);
...
//jdb2018-08-21 保存打印文本
void SavePrintStr(BYTE flag, CONSTCHAR *str)
{
	int i, j;
	int datalen;
	...
	recount = 0;
	j = strlen(PrBuffer);
	if(j + datalen >= sizeof(PrBuffer))
		return;
	
	PrBuffer[j++] = flag & 0xff;
	for(i = 0; i < datalen; i++)
	{
		if(databuf[i] == oldchar)
		{
			recount++;
			if(i == datalen - 1)
			{
				PrBuffer[j++] = 0x80 | (recount & 0xff);
				recount = 0;
			}
		}
		else if(recount > 0)
		{
			...
			PrBuffer[j++] = databuf[i] & 0xff;
			oldchar = databuf[i] & 0xff;
		}
		else
		{
			PrBuffer[j++] = databuf[i] & 0xff;
...
//                StoreEJEnd();
    ...
    if (ApplVar.FReceipt)    /* Receipt Used ? */
    {
        ...
        else
        {
			...
			//jdb2018-08-21 初始化BUFFER
			memcpy(PrBuffer2, PrBuffer, strlen(PrBuffer));
			PrBuffer2[strlen(PrBuffer)] = '\0';
			PrBuffer[0] = '\0';
			Receipt_Data(RECEIPTLOG);//jdb2018-08-22 打印票据文本结束
