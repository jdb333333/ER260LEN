...
extern void SavePrintStr(BYTE flag, CONSTCHAR *str);
...
{
	char spacebuf[PRTLEN + 1];
	
	if(ApplVar.CentralLock == RG)
		{
			if(Line)
				SavePrintStr(TESTBIT(ApplVar.PrintLayOut,BIT2)?CMDP_DR:CMDP_R, Line);//jdb2018-08-21 保存打印文本
			else
			{
				memset(spacebuf, ' ', PRTLEN);
				spacebuf[PRTLEN] = '\0';
